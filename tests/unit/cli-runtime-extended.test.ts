...
function writeScript(dir, name, content, executable = true) {
  ...
  // Skip chmod on Windows - it's not needed and can cause permission errors
  if (process.platform !== "win32") {
    try {
      fs.chmodSync(filePath, executable ? 0o755 : 0o644);
    } catch (error) {
      // Ignore chmod errors - the file is still usable
      console.warn(`chmod failed for ${filePath}:`, error);
    }
  }
  return filePath;
}
