...
import { RAW_CAP, MIN_COMPRESS_SIZE, type RtkStats } from "./constants.js";
import { autoDetectFilter } from "./autodetect.js";
import { safeApply } from "./applyFilter.js";

// Compress tool_result content in-place. Returns stats or null if disabled/failed.
export function compressMessages(body: any, enabled: boolean): RtkStats | null {
  ...
  try {
    ...
    console.warn("[RTK] compressMessages error:", (e as Error).message);
    return null;
  }
  return stats;
}
...
function compressKiroFormat(body: any, enabled: boolean): RtkStats | null {
  const stats: RtkStats = { bytesBefore: 0, bytesAfter: 0, hits: [] };
  try {
    ...
    console.warn("[RTK] compressKiroFormat error:", (e as Error).message);
    return null;
  }
  return stats;
}
